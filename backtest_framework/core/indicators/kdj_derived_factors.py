...
def calculate_buy_signal_counter(data: pd.DataFrame, max_signals_per_cross: int = 2) -> pd.DataFrame:
    """
    Track buy signals per golden cross period and limit signal generation.
    
    This prevents over-trading by limiting the number of buy signals
    that can be generated during each golden cross period.
    
    Args:
        data: DataFrame with golden_cross, bars_since_golden_cross columns
        max_signals_per_cross: Maximum buy signals per golden cross period (default: 2)
        
    Returns:
        DataFrame with buy_signals_since_cross, can_generate_buy columns
    """
    # Ensure required columns exist
    required_cols = ['golden_cross', 'bars_since_golden_cross']
    for col in required_cols:
        if col not in data.columns:
            raise ValueError(f"Required column '{col}' not found in data. Available columns: {list(data.columns)}")
    ...
    # Initialize tracking
    signals_since_cross = np.zeros(len(data))
    can_generate_buy = np.ones(len(data))  # Start with ability to generate signals
    
    current_signal_count = 0
    
    for i in range(len(data)):
        # Reset counter on new golden cross
        if data['golden_cross'].iloc[i] == 1:
            current_signal_count = 0
            
        # Update signal count and permission
        signals_since_cross[i] = current_signal_count
        can_generate_buy[i] = 1 if current_signal_count < max_signals_per_cross else 0
        
        # This would be incremented externally when a buy signal is actually generated
        # For now, we just track the potential
    
    # Assign to result DataFrame
    result['buy_signals_since_cross'] = signals_since_cross
    result['can_generate_buy'] = can_generate_buy
